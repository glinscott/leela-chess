...
class TFProcess:
    ...
    def init_net(self, planes, probs, winner):
        self.x = planes  # (tf.float32, [None, 120, 8 * 8])
        self.y_ = probs  # (tf.float32, [None, 1924])
    ...
    def measure_loss(self, batch, training=False):
        ...
        r = self.session.run(ops, feed_dict={self.learning_rate: self.lr, self.training: training,
    ...
    def process(self, train_data, test_data):
        ...
        if steps % self.info_steps == 0:
            speed = self.info_steps * self.batch_size / self.timer.elapsed()
            print("step {}, lr={:g} policy={:g} mse={:g} reg={:g} total={:g} ({:g} pos/s)".format(
                steps, self.lr, self.stats.mean('policy'), self.stats.mean('mse'), self.stats.mean('reg'),
                self.stats.mean('total'), speed))
            summaries = self.stats.summaries({'Policy Loss': 'policy', 'MSE Loss': 'mse'})
